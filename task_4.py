...
# degree chosen was 3, no need to do loops
def validate_knn_regression(X_train, y_train, X_val, y_val, regressor=None, k=range(1, 15), max_features=None):    
    bestRmse = 2
    bestModel = None
    bestK = None
    
    preprocessing = ColumnTransformer(transformers=[('features', FunctionTransformer(createFeatures, validate=False), ['x1_initial_position', 'y1_initial_position', 'x2_initial_position',
                                    'y2_initial_position', 'x3_initial_position', 'y3_initial_position', 't'])], 
                                    remainder='passthrough')
    # loop k
    for kVal in k:
        knnModel = make_pipeline(preprocessing, KNeighborsRegressor(n_neighbors=kVal))
        knnModel.fit(X_train, y_train)
        
        yPred = knnModel.predict(X_val)
        rmse = np.sqrt(mean_squared_error(y_val, yPred))
        
        print(f"K {kVal}, RMSE: {rmse}")
    
        # Update the best model and RMSE score if necessary
        if rmse < bestRmse:
            bestModel = knnModel
            bestRmse = rmse
            bestK = kVal

    print(f"Best Degree: {bestK}, Best RMSE: {bestRmse}")
    return bestModel, bestRmse, bestK

bestModel, bestRmse, bestK = validate_knn_regression(entryTrain, outputTrain, entryVal, outputVal)
...
plot_y_yhat(np.array(outputVal), outputValPrediction, plot_title="y_yhat_val_knn")
plot_y_yhat(np.array(outputTrain), outputTrainPrediction, plot_title="y_yhat_train_knn")
